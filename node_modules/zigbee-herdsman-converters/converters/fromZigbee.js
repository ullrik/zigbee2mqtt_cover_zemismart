    ...
    ZMCSW032D_cover_position_tilt: {
        ...
        convert: (model, msg, publish, options, meta) => {
            ...
            if (options.hasOwnProperty('time_close') && options.hasOwnProperty('time_open')) {
                ...
                console.log(`ZMCSW032D data lastPreviousAction : ${lastPreviousAction} `);
                console.log(`ZMCSW032D data currentPositionLiftPercentage : ${msg.data['currentPositionLiftPercentage']} `);
                console.log(`ZMCSW032D data newLastPreviousAction : ${store[deviceID].lastPreviousAction} `);
                console.log(`ZMCSW032D data store[deviceID].since : ${store[deviceID].since} `);
                console.log(`ZMCSW032D data deltaTimeSec : ${deltaTimeSec} `);
                console.log(`ZMCSW032D data time_close : ${options.time_close} `);
                console.log(`ZMCSW032D data time_open : ${options.time_open} `);

                if (msg.data.hasOwnProperty('currentPositionLiftPercentage') &&
                    msg.data['currentPositionLiftPercentage'] == 50 ) {
                    console.log(`    ZMCSW032D data currentPositionLiftPercentage == 50 `);
                    if (deltaTimeSec < timeCoverSetMiddle || deltaTimeSec > timeCoverSetMiddle) {
                        console.log(`        ZMCSW032D data acttion != 60`);                    
                        if (lastPreviousAction == 100 ) {
                            // Open
                            console.log(`            ZMCSW032D data time_open : ${options.time_open} `);
                            console.log(`            ZMCSW032D data currentPosition 1 : ${currentPosition} `);
                            currentPosition = currentPosition == -1 ? 0 : currentPosition;

                            console.log(`            ZMCSW032D data currentPosition 2 : ${currentPosition} `);
                        ...
                        console.log(`        ZMCSW032D data currentPosition 3 : ${currentPosition} `);
                        currentPosition = currentPosition > 100 ? 100 : currentPosition;
                        console.log(`        ZMCSW032D data currentPosition 4 : ${currentPosition} `);
                        currentPosition = currentPosition < 0 ? 0 : currentPosition;
                        console.log(`        ZMCSW032D data currentPosition 5 : ${currentPosition} `);
                    }
                }
                store[deviceID].CurrentPosition = currentPosition;
                console.log(`ZMCSW032D data position : ${store[deviceID].CurrentPosition} `);
