...
const converters = {
    ...
    ZMCSW032D_cover_position_tilt: {
        ...
        convertSet: async (entity, key, value, meta) => {
            ...
            const isPosition = (key === 'position');
            //const oldPosition = store[entity.deviceIeeeAddress].CurrentPosition;
            //console.log(`ZMCSW032D toZigbee oldPosition : ${oldPosition} `);
            console.log(`ZMCSW032D toZigbee isPosition : ${isPosition} `);
            console.log(`ZMCSW032D toZigbee entity : ${JSON.stringify(entity)} `);
            console.log(`ZMCSW032D toZigbee key : ${key} `);
            console.log(`ZMCSW032D toZigbee value : ${value} `);
            /*var str = '';
            for (var p in meta) {
                if (meta.hasOwnProperty(p)) {
                    var infoMeta = p + '::' + meta[p] + '\n';
                    console.log(`ZMCSW032D toZigbee meta : ${infoMeta} `);
                    if ( p != "logger") {
                        console.log(`ZMCSW032D toZigbee meta : ${JSON.stringify(meta[p])} `);
                    }
                }
            }*/
            ...
            console.log(` `);
            console.log(`ZMCSW032D toZigbee time_close : ${meta.options.time_close} `);
            console.log(`ZMCSW032D toZigbee time_open : ${meta.options.time_open} `);
            console.log(`ZMCSW032D toZigbee oldPosition : ${oldPosition} `);
            console.log(`ZMCSW032D toZigbee value : ${value} `);
            console.log(` `);
            if (value == 100) {
                console.log(`    ZMCSW032D toZigbee open `);
                await entity.command('closuresWindowCoveringZMCSW032D', "open", {}, getOptions(meta.mapped));
            } else if (value == 0) {
                console.log(`    ZMCSW032D toZigbee close `);
                await entity.command('closuresWindowCoveringZMCSW032D', "close", {}, getOptions(meta.mapped));
            } else {
                if (oldPosition > value) {
                    const delta = oldPosition - value;
                    console.log(`        ZMCSW032D toZigbee delta : ${delta} `);
                    let mutiplicateur = meta.options.time_open/100;
                    console.log(`        ZMCSW032D toZigbee mutiplicateur : ${mutiplicateur} `);
                    const timeBeforeStop = delta*mutiplicateur;
                    console.log(`        ZMCSW032D toZigbee timeBeforeStop : ${timeBeforeStop} `);
                    await entity.command('closuresWindowCoveringZMCSW032D', "close", {}, getOptions(meta.mapped));
            ...
            // HomeAssistant etc. work the other way round.
            // But e.g. Legrand expects "open" to be 100 and "closed" to be 0
            /*let test = {state: {[isPosition ? 'position' : 'tilt']: value}};
            console.log(`ZMCSW032D toZigbee test : ${JSON.stringify(test)} `);
            value = 100;
            let test2 = {"Shutter1":{"Position":0,"direction":0}};
            console.log(`ZMCSW032D toZigbee test2 : ${JSON.stringify(test2)} `);
            await entity.write('closuresWindowCovering', "OPEN", getOptions(meta.mapped, entity));
            */
            /*await entity.command(
                'closuresWindowCovering',
                isPosition ? 'goToLiftPercentage' : 'goToTiltPercentage',
